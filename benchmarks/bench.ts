...
type BenchSuiteResult = {
  [name: string]: { hz: number; elapsed: number; rme: number; mean: number };
};
...
export class BenchSuite {
  ...
  constructor(public name: string, protected maxTime: number = 1) {
    this.suite
      .on('complete', () => {
        ...
        print(' 🏁 Fastest', green(fastest.name));
      })
      .on('cycle', function (event: any) {
        print(
          ...
          'x',
          green(
            event.target.hz.toLocaleString(undefined, {
          ...
          yellow(
            event.target.stats.mean.toLocaleString(undefined, {
  ...
  addAsync(title: string, fn: () => Promise<void>, _options: any = {}) {
...
export async function bench(
  ...
  process.stdout.write(
    [
      ...
      ops((1000 / took) * times),
      green(title),
      took.toLocaleString(undefined, { maximumFractionDigits: 17 }),
      'ms,',
      (took / times).toLocaleString(undefined, { maximumFractionDigits: 17 }),
      'ms per item',
